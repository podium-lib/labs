...
export default class ProcessExceptionHandlers {
    constructor(logger) {
    ...
    terminate(exitCode = this.exitCodeToUse, exiter = process.exit) {
        ...
        const id =
            this[timeout] === 0
                ...
                : setTimeout(() => {
                      exited = true;
                      this.logger.error(
                          `Termination timed out in ${this[timeout]}.`,
                      );
        ...
        this.deathrow.forEach(cb => {
            try {
                ...
                this.logger.error(
                    'Got error when invoking shutdown callback',
                    e,
                );
                resolver();
            }
    ...
    functionOnExit(func, server, options) {
        ...
        const shutdown = done => {
            ...
            server[func](err => {
                ...
                if (err) {
                    this.logger.error(
                        `Failed closing down in ${milliseconds}ms.`,
                        err,
                    );
                    ...
                    this.logger.info(
                        `Successfully closed down in ${milliseconds}ms.`,
                    );
